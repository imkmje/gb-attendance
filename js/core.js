...
const CHANGELOG = [
  { v:'3.13.2', d:'2026-09-21', t:'patch', title:'그린라이트 배지·사유 옵션에서 🟢 이모지 제거 — 다른 명단 배지("결석 N회" 등)와 같은 무채색 텍스트 pill 스타일로 통일(예: "그린라이트 2개")' },
  { v:'3.13.1', d:'2026-09-21', t:'patch', title:'PC 출석체크 카드 그리드를 고정 5·6열에서 카드 최소 폭(210px) 보장 방식으로 교체 — 화면이 넓을수록 카드가 좁아져 "결석 N회" 배지가 줄바꿈되던 v3.13.0 임시 수정(배지를 다음 줄로 내림)을 원복하고, 대신 카드 자체를 넉넉하게 유지해 한 줄에 그대로 보이게 함' },
  { v:'3.13.0', d:'2026-09-21', t:'minor', title:'그린라이트 개별 조정 신설 + PC 출석체크 결석 배지 줄바꿈 수정 — ① 개발자 메뉴에 엑셀 없이 이름 검색만으로 학생 한 명에게 바로 부여·회수하는 화면 추가(일괄 지급과 같은 매칭·활동 로그 로직 재사용), ② PC(가로 화면)에서 카드 폭이 좁아지면 "결석 N회" 배지 안에서 글자가 줄바꿈되던 문제 수정(배지는 항상 한 줄 유지, 자리가 부족하면 배지 전체가 다음 줄로 내려감)' },
  { v:'3.12.1', d:'2026-09-21', t:'patch', title:'그린라이트 일괄 지급 화면에서 회수(차감)도 가능하도록 개선 — 붙여넣는 개수 앞에 -를 붙이면 회수로 처리(예: 잘못 지급한 경우 정정), 출처 선택에 "정정·회수" 추가, 0 밑으로는 안 내려감' },
  { v:'3.12.0', d:'2026-09-21', t:'minor', title:'"그린라이트" 기능 신설 — 내신·모의고사 목표를 달성한 학생에게 지급되는 결석 허용권. 개발자 메뉴 → 데이터 탭에서 엑셀 반·번호·이름·개수를 붙여넣어 일괄 지급(기존 개수에 더해짐), 잔여 개수가 있는 학생에게만 출석체크 결석 사유 맨 위에 "그린라이트 사용"이 자동으로 뜨고, 선택해 저장하면 그날 결석 카운트에서 완전히 빠지면서(노카운트 자동 적용) 개수가 1개 차감·목록에서 사라짐. 나중에 사유를 다른 걸로 고치거나 기록을 삭제하면 개수 자동 환불, 명단 탭 카드에 잔여 개수 배지 표시' },
  { v:'3.11.0', d:'2026-08-27', t:'minor', title:'개발자 메뉴를 반투명 배경에 뜨는 팝업(바텀시트)에서 화면 전체를 덮는 페이지 전환으로 변경 — 데이터·상태는 그대로(같은 탭 안에서 전환), 배경 딤·둥근 모서리만 제거해 별도 화면처럼 보이게' },
  { v:'3.10.0', d:'2026-08-27', t:'minor', title:'개발자 메뉴 결석 사유 관리 — "출석체크 화면에 표시" 꺼둔(숨긴) 사유를 목록에서 분리해 하단 "숨긴 사유 (N)" 접이식 섹션으로 모음. 안 쓰는 사유가 쌓여도 주 목록이 길어지지 않으면서, 삭제 대신 숨김을 쓰도록(과거 통계 소급 영향 방지) 유도' },
  { v:'3.9.1', d:'2026-08-25', t:'patch', title:'대시보드 카드를 꾹 눌러 출석 기록 수정을 열 때, 모바일 브라우저가 꾹 누르기를 텍스트 선택으로 오인해 파란 블록이 잡히던 문제 수정(user-select:none 적용) — 위반 등록에 쓰던 명단 탭 카드와 동일하게 선택 자체를 막음' },
  ...
  { v:'3.2.0', d:'2026-08-20', t:'minor', title:'"출석 인정" 사유 기능 다듬기 3건 — ① 개발자 메뉴에서 사유의 "출석률 계산 시 출석 인정" 설정을 바꿔도 통계 탭에 예전 캐시된 출석률이 최대 5분간 그대로 남아있던 문제 수정, ② 앱을 막 켜자마자 통계 탭에 들어가면 사유 설정이 로드되기 전 기본값으로 계산돼버리던 경합 상태 수정, ③ 사유별 "출석체크 화면에 표시" 스위치 추가 — 더는 안 쓰는 사유는 삭제(과거 기록의 출석 인정 설정이 소급으로 풀림) 대신 숨김 처리로 사유 선택 목록에서만 뺄 수 있음' },
  { v:'3.1.0', d:'2026-08-20', t:'minor', title:'개발자 메뉴 → 결석 사유 관리에 "출석률 계산 시 출석 인정" 스위치 추가 — 자습 시간과 학교 자체 프로그램(예: 청백지교)이 겹쳐 결석 처리한 경우, 자습 누적 시간에서는 그대로 빠지되 통계·학생 인사이트·기간 결산의 출석률 계산에서는 출석으로 반영되게 사유별로 설정 가능. 출석체크 화면의 사유 선택 목록에는 이 속성이 드러나지 않음' },
  { v:'3.0.4', d:'2026-08-20', t:'patch', title:'교사 메뉴 "결석 카운트 수정"이 하루에 세션 2개(예: 야간+심야 일괄 적용) 결석이면 1일이 아니라 2회로 잘못 세던 버그 수정 — 통계·학생 인사이트·기간 결산과 같은 "하루 대표 세션" 기준으로 통일' },
...
function _bindCardTapAndHold(el, onTap, onLongPress) {
  // .roster-card(위반 등록 롱프레스)와 동일하게 — 이게 없으면 모바일
  // 브라우저가 꾹 누르기를 텍스트 선택으로 해석해 복사/공유 팝업을 띄운다.
...
let _holidays = [];
// countsAsPresent: true면 이 사유로 결석 처리해도 출석률 계산 시 출석으로
// 인정한다(예: 자습 시간과 겹치는 학교 자체 프로그램 참여). 자습 누적 시간·
// 최근 결석 이력 표시에는 영향 없음 — 실제로 자습을 안 한 건 그대로 반영됨.
// visible: false면 출석체크 화면의 사유 선택 목록에서만 감춘다(설정·과거
// 기록은 유지) — 더는 안 쓰는 사유는 삭제 대신 이걸로 숨기는 걸 권장한다.
// 삭제하면 _reasonTypes에서 항목 자체가 사라져 과거 기록의 출석 인정
// 재분류가 조용히 풀려버리기 때문(지난 기간 출석률이 소급 재계산됨).
// 개발자 메뉴 "결석 사유 관리"에서 사유별로 켜고 끌 수 있다.
let _reasonTypes = [
  { name: '학원 보강',   countsAsPresent: false, visible: true },
  { name: '병결',        countsAsPresent: false, visible: true },
  { name: '개인 사정',   countsAsPresent: false, visible: true },
];
// getReasonTypes()가 실제 서버 값으로 _reasonTypes를 채운 뒤 resolve되는
// Promise. 통계 탭처럼 "출석 인정" 설정이 계산 결과에 영향을 주는 화면은
// 이 값을 기다린 뒤에 계산해야, 페이지를 막 열자마자 눌러도 아직 로드
// 전인 기본값(전부 꺼짐)으로 잘못 계산되는 걸 막을 수 있다.
let _reasonTypesReadyPromise = null;
let _activityLogOn = true;
let _cardExportOn = true; // 기간 결산 "카드 이미지로 내보내기" 기능 — 개발자 메뉴에서 전역 on/off, 서버에서 로드 전엔 기본 켜짐
let _afterSchoolToggleOn = true; // 오후 자율학습 화면의 "방과후 없는 날" 토글 — 개발자 메뉴에서 전역 on/off, 서버에서 로드 전엔 기본 켜짐
