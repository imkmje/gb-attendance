...
function _openStudentInsightSheet(student) {
  ...
  Promise.resolve(_reasonTypesReadyPromise)
    .then(() => API.getStudentInsight(student.id, _reasonTypes.filter(r => r.countsAsPresent).map(r => r.name)))
    .then(insight => {
...
function openPeriodSummarySheet() {
  ...
  const runQuery = () => {
    ...
    Promise.resolve(_reasonTypesReadyPromise)
      .then(() => API.getPeriodSummary(start, end, _reasonTypes.filter(r => r.countsAsPresent).map(r => r.name)))
      .then(data => {
        summary = data || [];
